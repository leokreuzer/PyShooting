...
def rungekutta4(X, system, ti_parameters, fex_values):
    ...
    if n == 1:
        ...
        def phi(tau, y, Om):
            r = Dual.dot(statespace_matrix, y) - np.array([0, system.f_nl(y) - fex_values[int(2*tau)]])*(1/(M*Om**2))
            return r
    elif n>1:
        statespace_matrix = np.vstack([np.hstack([np.zeros([n,n]), np.eye(n)]), np.hstack([(-1)*np.dot(inv(M), K)*(1/(Om**2)), (-1)*np.dot(inv(M), C)*(1/Om)])])
        def phi(tau, y, Om):
            r = Dual.dot(statespace_matrix, y) - np.hstack([np.zeros(n), np.dot(inv(M) ,system.f_nl(y*Om) - fex_values[int(2*tau),:])*(1/(Om**2))])
            return r
    ...
    for j in range(ti_parameters.Np*ti_parameters.Ntpp):
        ...
        k1 = phi(j, y, Om)
        k2 = phi((j+0.5), y+0.5*k1*dtau, Om)
        k3 = phi((j+0.5), y+0.5*k2*dtau, Om)
        k4 = phi((j+1), y+k3*dtau, Om)
...
##### uncomment, if you have a statespace function given(including precomputed derivatives)
...
##### uncomment, if you have a statespace function given(without precomputed derivatives)
# def rungekutta4(X, ti_parameters, n):

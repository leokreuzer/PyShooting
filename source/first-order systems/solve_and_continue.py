...
def fra(X0, n, ti_parameters, solver_parameters, N_sol, steplength_parameters):
    ...
    X_sol[0,:], X, Jacobian_sol[0,:,:], Y_sol[0,:,:], number_iterations[0] = solver.initial(X0, n, ti_parameters, solver_parameters)
    ...
    for sk in range(N_sol-1):
        run_var = 1
        while run_var:
            X_pred, X, p_sol[sk,:], steplength[sk] = predictor.tangent_fra(Jacobian_sol[sk, :, :], X_sol[sk,:], p_sol[sk-1,:], steplength[sk], n, sk, X_sol[sk-1], steplength_parameters.initial_direction)

            X_sol[sk+1,:], X, Jacobian_sol[sk+1,:,:], Y_sol[sk+1,:,:], number_iterations[sk+1], run_var = corrector.orthogonal(X, X_pred, p_sol[sk,:], n, ti_parameters, solver_parameters, steplength[sk], X_sol[sk, :])
...
def nma(X0, system, ti_parameters, solver_parameters, Fex, N_sol, steplength_parameters):
    ...
    print(f"1/{N_sol}\n{round(float((1/N_sol)*100), 3)}% done")
    ...
    for sk in range(N_sol-1):
        run_var = 1
        while run_var:
            X_pred, X, p_sol[sk,:], steplength[sk] = predictor.tangent_nma(Jacobian_sol[sk, :, :], X_sol[sk,:], p_sol[sk-1,:], steplength[sk], n, sk, X_sol[sk-1], steplength_parameters.initial_direction)
        ...
        print(f"{sk+2}/{N_sol}\n{round(float(((sk+2)/N_sol))*100, 3)}% done")
        print(X_sol[sk+1, -1])
